...
import org.example.utilidades.FiltroFecha
import java.util.*
...
class ConsolaUI(
    ...
    fun mostrarMenuFiltrado() {
        ...
        val nombreUsuario = readLine()?.trim()?.takeIf { it.isNotBlank() }
        ...
        val fechaInput =
            readLine()
                ?.trim()
                ?.uppercase()
                ?.takeIf { it in listOf("HOY", "MAÑANA", "SEMANA", "MES") }

        // Conversión String? -> FiltroFecha?
        val fechaFiltro: FiltroFecha? =
            try {
                fechaInput?.let { FiltroFecha.valueOf(it) }
            } catch (e: Exception) {
                null
            }
